...
beforeEach(function () {
    Cache::forget('clickpesa_access_token');
});

function fakeClickpesaToken(): void
{
    Http::fake([
...
test('initiatePayment sends correct payload and returns transactionId', function () {
    Http::fake([
        'api.clickpesa.com/third-parties/non-trade/users/generate-token' => Http::response(['token' => 'Bearer eyJfake'], 200),
    ...
    Http::assertSent(function ($request) {
        return str_contains($request->url(), 'initiate-ussd-push-request')
...
test('initiatePayment normalizes phone number starting with 0', function () {
    ...
    Http::assertSent(fn ($r) => ($r->data()['phoneNumber'] ?? null) === '255765000000');
});
...
test('getAccessToken is cached and reused across calls', function () {
    Http::fake([
        'api.clickpesa.com/third-parties/non-trade/users/generate-token' => Http::response(['token' => 'Bearer eyJcached'], 200),
        'api.clickpesa.com/third-parties/payments/initiate-ussd-push-request' => Http::response([
